...
var map; 
	...
	function initialize(source/*archivo xml*/, target/*capa en la que se cargaran los datos tabulados*/, del /*op eliminar*/) {
	  if (GBrowserIsCompatible()) {
		...
		document.getElementById('precarga').innerHTML = 'Cargando Puntos Zonas de Riesgo Inundacion.....';
    	...
    	map.addOverlay(marker); 
			...
			GEvent.addListener(map, "mousemove", function (mev){
			    var html='<strong>Riesgo INUNDACION MANATI</strong><br/>Mouse: Y=' + mev.lat().toFixed(6) + ', X=' + mev.lng().toFixed(6) + '<br/>'+ 'Zoom: '+map.getZoom() + '<br/> Centro Mapa: Y='+ map.getCenter().lat().toFixed(6) + ', X=' + map.getCenter().lng().toFixed(6);
			  ...
			  for (var i = 0; i < records.length; i++) {
				 ...
				 /* var obama = parseInt(rec.getAttribute("Obama"));
				  ...
				  map.addOverlay(poly);
					...
					document.getElementById('precarga').innerHTML = 'Riesgo Inundacion Manati: <strong>'+ contador + '</strong> Poligonos Cargados';
...
function preload()
{	
	initialize('xml/inundacionmanati.xml','side_bar',0);
